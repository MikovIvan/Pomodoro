...
import ru.mikov.pomodoro.R.styleable
...
class CountDownView @JvmOverloads constructor(
    ...
    init {
        if (attrs != null) {
            val styledAttrs = context.theme.obtainStyledAttributes(
                attrs,
                styleable.CountDownView,
                defStyleAttr,
                0
            )
            color = styledAttrs.getColor(styleable.CountDownView_custom_color, Color.RED)
            style = styledAttrs.getInt(styleable.CountDownView_custom_style, FILL)
            styledAttrs.recycle()
        }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawArc(
            0f, 0f,
            width.toFloat(), height.toFloat(), -90f, startAngel, true, paint
        )
